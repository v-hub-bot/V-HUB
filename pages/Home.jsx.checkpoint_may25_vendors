    ...
    <div style={{ padding: "8px 12px 12px" }}>
      ...
      <div style={{ marginBottom: 8, textAlign: "center" }}>
        <input
          ...
          style={{
            padding: "5px 12px", fontSize: 13, border: "1px solid #C8A96E",
            borderRadius: 20, background: "#FFFDF5", width: "90%", maxWidth: 320,
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 6 }}>
          {filtered.map(v => (
            <div key={v.id} onClick={() => setSelectedVendor(v)} style={{
              background: "#FFFDF5", border: "1px solid #C8A96E",
              borderRadius: 6, padding: "8px 10px", cursor: "pointer",
              transition: "box-shadow 0.15s",
              boxShadow: "0 1px 3px rgba(0,0,0,0.06)",
            }}
              onMouseEnter={e => e.currentTarget.style.boxShadow = "0 3px 10px rgba(232,67,26,0.2)"}
              onMouseLeave={e => e.currentTarget.style.boxShadow = "0 1px 3px rgba(0,0,0,0.06)"}
            >
              <div style={{ fontSize: 15 }}>{VENDOR_CAT_EMOJI[v.category] || "🏪"}</div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#3A1A0A", fontFamily: "'Times New Roman', serif", marginTop: 2, lineHeight: 1.3 }}>
                {v.name}
              </div>
      ...
      <div style={{ marginTop: 12, textAlign: "center" }}>
        <a href="/VendorSignup" style={{
          display: "inline-block", padding: "7px 18px", background: "#E8431A",
          color: "#fff", borderRadius: 20, fontSize: 12, fontWeight: 700,
          textDecoration: "none", fontFamily: "'Times New Roman', serif",
          letterSpacing: 0.5, boxShadow: "0 2px 6px rgba(232,67,26,0.3)",
        }}>
      ...
      {/* Vendor Detail Modal */}
      {selectedVendor && (
        <div onClick={() => setSelectedVendor(null)} style={{
          position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)",
          zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center",
          padding: 16,
        }}>
          <div onClick={e => e.stopPropagation()} style={{
            background: "#FFFDF5", borderRadius: 10, padding: "22px 20px",
            maxWidth: 380, width: "100%", boxShadow: "0 8px 32px rgba(0,0,0,0.25)",
            border: "2px solid #C8A96E", fontFamily: "'Times New Roman', serif",
            ...
            <button onClick={() => setSelectedVendor(null)} style={{
              ...
              border: "none", fontSize: 22, cursor: "pointer", color: "#888", lineHeight: 1,
            }}>×</button>
            <div style={{ fontSize: 28, textAlign: "center", marginBottom: 6 }}>
              {VENDOR_CAT_EMOJI[selectedVendor.category] || "🏪"}
            </div>
            <h2 style={{ margin: "0 0 4px", fontSize: 18, fontWeight: 900, color: "#3A1A0A", textAlign: "center", lineHeight: 1.3 }}>
              {selectedVendor.name}
            </h2>
            <div style={{ textAlign: "center", marginBottom: 10 }}>
              <span style={{ background: "#E8431A22", color: "#E8431A", fontSize: 11, fontWeight: 700, padding: "2px 10px", borderRadius: 12, border: "1px solid #E8431A55" }}>
                {selectedVendor.category}
              </span>
            </div>
            <div style={{ borderTop: "1px solid #C8A96E", paddingTop: 10 }}>
              {selectedVendor.description && (
                <p style={{ fontSize: 13, color: "#5C3A1E", marginBottom: 10, lineHeight: 1.5 }}>
                  {selectedVendor.description}
                </p>
              )}
              <div style={{ fontSize: 12, color: "#5C3A1E", lineHeight: 1.8 }}>
                <div>📍 <strong>Location:</strong> {selectedVendor.location || "The Villages, FL"}</div>
                <div>🕘 <strong>Schedule:</strong> {selectedVendor.schedule || "Every Saturday 9:00 AM – 1:00 PM at Brownwood Paddock Square"}</div>
                {selectedVendor.phone && <div>📞 <strong>Phone:</strong> <a href={`tel:${selectedVendor.phone}`} style={{ color: "#E8431A" }}>{selectedVendor.phone}</a></div>}
                {selectedVendor.email && <div>✉️ <strong>Email:</strong> <a href={`mailto:${selectedVendor.email}`} style={{ color: "#E8431A" }}>{selectedVendor.email}</a></div>}
                {selectedVendor.website && <div>🌐 <strong>Website:</strong> <a href={selectedVendor.website} target="_blank" rel="noopener noreferrer" style={{ color: "#E8431A" }}>{selectedVendor.website}</a></div>}
                {selectedVendor.facebook_url && <div>📘 <strong>Facebook:</strong> <a href={selectedVendor.facebook_url} target="_blank" rel="noopener noreferrer" style={{ color: "#E8431A" }}>View Page</a></div>}
              </div>
            </div>
            <div style={{ marginTop: 14, textAlign: "center", fontSize: 11, color: "#aaa", fontStyle: "italic" }}>
