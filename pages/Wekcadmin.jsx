...
function AnalyticsTab({ providers, reviews, leads, stats, catMap, svcMap, fullSvcMap, classifiedAds }) {
  ...
  const [gaLoading, setGaLoading] = React.useState(true);

  // GA4 data - direct link (connector not yet wired to this property)
  React.useEffect(() => {
    setGaLoading(false);
    setGaData(null);
  }, [timeRange]);
  ...
  const activeAds = (classifiedAds||[]).filter(a=>{if(!a.is_active)return false;if(a.deal_expires_at){const exp=new Date(a.deal_expires_at);if(exp<now)return false;}return true;});
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      {/* GA4 Site Traffic */}
      <div style={S.card}>
        <div style={S.secTitle}>🌐 Site Traffic — {rangeLabels[timeRange]}</div>
        {gaLoading ? (
          <div style={{ fontSize:13, color:T.brownLight, fontFamily:T.sans, padding:"10px 0" }}>Loading traffic data...</div>
        ) : (
          <div style={{ fontSize:13, fontFamily:T.sans }}>
            <div style={{ marginBottom:10, color:T.brownDark }}>View your full traffic data directly in Google Analytics:</div>
            <a href="https://analytics.google.com/analytics/web/#/p490233278/reports/reportinghub" target="_blank" rel="noopener noreferrer"
            ...
            <div style={{ marginTop:8, fontSize:11, color:"#aaa" }}>Property: G-1EJ40FW9E1 · www.v-hub.us</div>
          </div>
        )}
      </div>
      ...
      <div style={S.card}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
          {[
            ["New Providers", filteredProviders.length, T.teal],
            ["New Leads", filteredLeads.length, "#1A6B3C"],
