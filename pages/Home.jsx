...
function ProvDetail({ prov, areas, cats, svcs, onBack }) {
  ...
  const handleReviewSubmit = async () => {
    if (!reviewForm.review_text || !reviewForm.rating) {
      alert("Please write your review before submitting.");
      return;
    }
    try {
      // Never store or display the customer's name — privacy policy
...
export default function Home() {
  ...
  const doSearch = async (selSvc, selArea) => {
    ...
    // ── Step 1: Fetch providers + lookup data via backend (service role — works without auth) ──
    try {
      ...
      // Enrich service/area maps from lookup data
      svcEntities = lookupData.services || [];
      (lookupData.areas || []).forEach(a => {
        if (a.id && a.name) {
